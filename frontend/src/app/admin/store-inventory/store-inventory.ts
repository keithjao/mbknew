...
export class StoreInventory implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.operationsStore.operations$.subscribe(() => {
        this.todaysOperation = this.operationsStore.getTodaysOperation('store');
        this.ensurePageBounds();
      })
    );
    ...
    this.subscription.add(
      this.operationsStore.auditLog$.subscribe(logs => {
        this.auditLogs = this.operationsStore.getAuditLogsByType('store');
        this.ensurePageBounds();
      })
    );
  }
  ...
  openOperation(): void {
    ...
    this.selectedItems.forEach((quantity, inventoryItemId) => {
      ...
      if (masterItem && quantity > 0) {
        items.push({
          id: `store-${masterItem.id}-${Date.now()}`,
          inventoryItemId,
    ...
    try {
      this.operationsStore.openOperation('store', items, this.currentUser, this.operationNotes);
      this.feedback = 'Store operation opened successfully!';
  ...
  closeOperation(): void {
    ...
    this.feedback = 'Store operation closed. Remaining items returned to master inventory.';

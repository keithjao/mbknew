...
@Component({
  selector: 'app-popup-inventory',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './popup-inventory.html',
  styleUrls: ['./popup-inventory.scss']
})
export class PopupInventory implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.operationsStore.operations$.subscribe(() => {
        this.todaysOperation = this.operationsStore.getTodaysOperation('popup');
        this.ensurePageBounds();
      })
    );
    ...
    this.subscription.add(
      this.operationsStore.auditLog$.subscribe(logs => {
        this.auditLogs = this.operationsStore.getAuditLogsByType('popup');
        this.ensurePageBounds();
      })
    );
  }
  ...
  openOperation(): void {
    ...
    this.selectedItems.forEach((quantity, inventoryItemId) => {
      ...
      if (masterItem && quantity > 0) {
        items.push({
          id: `popup-${masterItem.id}-${Date.now()}`,
          inventoryItemId,
    ...
    try {
      this.operationsStore.openOperation('popup', items, this.currentUser, this.operationNotes);
      this.feedback = 'Pop-up operation opened successfully!';
  ...
  closeOperation(): void {
    ...
    this.feedback = 'Pop-up operation closed. Remaining items returned to master inventory.';
